...
/* Escapa string antes de interpolar em innerHTML (dados vêm da planilha). */
function esc(s){
...
function renderExec(d){
  ...
  el.innerHTML = '<b>'+esc(selMes.value)+'</b>: '+fmtKm(d.kmTotal)+' em '+d.veiculos
...
function renderTabelaSem(d){
  ...
  var linhas = d.semTabela.map(function(r){
    return '<tr><td>'+esc(r[0])+'</td>'
      +['S1','S2','S3','S4','S5'].map(function(_,i){ return '<td>'+fmt(r[i+1])+'</td>'; }).join('')
      +'<td>'+fmt(r[6])+'</td></tr>';
  }).join('');
...
function renderTabelaDet(d){
  var linhas = d.detalhe.map(function(r){
    ...
    return '<tr><td>'+esc(r[0])+'</td><td>'+esc(r[1])+'</td><td>'+esc(r[2])+'</td><td>'+esc(r[3])+'</td><td>'+esc(r[4])+'</td>'
      +'<td>'+fmt(r[5])+'</td><td>'+fmt(r[6])+'</td><td>'+fmt(r[7])+' km</td>'+delta
      +'<td style="color:'+stCor+';font-weight:700;">'+esc(r[9])+'</td></tr>';
  }).join('');
...
function renderConsolDet(d){
  ...
  var card = function(lbl, val, unit, atual, anterior, fmtAbs){
    ...
    if(anterior===undefined || anterior===null || anterior===0){
      deltaTxt = mesAnt ? ('— sem registros em '+esc(mesAnt)) : 'primeiro mês da série';
    } else {
      ...
      deltaTxt = '<span class="'+cls+'">'+sinal+' '+fmtAbs(Math.abs(dAbs))
        +' ('+sinal+' '+Math.abs(pct).toFixed(1)+'%)</span> vs '+esc(mesAnt);
    }
...
function comparaConsumo(t, ant){
  ...
  el.innerHTML = '<b>🔁 Comparativo '+esc(ant.mes)+' → '+esc(t.mes)+'</b>: KM '
...
function kpisConsumo(t, ant){
  var delta = ant ? ((t.delta>=0?'▲ ':'▼ ')+Math.abs(t.delta).toFixed(1)+'% vs '+esc(ant.mes)) : '—';
...
function tabelaConsumo(mes, ant){
  ...
  var linhas = rows.map(function(r){
    ...
    return '<tr><td>'+esc(r[0])+'</td><td>'+esc(r[1])+'</td><td style="color:'+corComb(r[2])+';font-weight:700;">'+esc(r[2])+'</td>'
