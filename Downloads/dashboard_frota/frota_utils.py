...
def chave_frota(mes_ref, ano=None) -> str:
    """Chave de mês da frota p/ juntar com o CO2: 'AGOSTO' -> 'AGOSTO 2026'.

    Quando o nome do mês não traz o ano, usa `ano` (padrão: o ano corrente).
    Isso evita hardcodar o ano de 2026 na lógica — a tabela de KM muda de ano
    conforme o ciclo de atualização mensal da planilha."""
    s = str(mes_ref).strip()
    if re.search(r"\d{4}$", s):
        return s
    return f"{s} {ano or datetime.now().year}"
...
def _limpar_br(s: str) -> float:
    """Converte texto numérico pt-BR/inglês p/ float:
    '12.727' -> 12727 (ponto como milhar, grupo de 3), '1,5'/'1.5' -> 1.5 (decimal).

    Regra p/ o caso ambíguo de um único ponto: se a parte fracionária tem 3 dígitos
    é encarado como milhar (valores de KM inteiros em pt-BR); até 2 dígitos é decimal.
    """
    s = s.strip()
    if "," in s:
...
def resumo_co2(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if df.empty:
        return pd.DataFrame()
    base = (df.groupby("MES_REF", as_index=False)
            .agg(MES_ORDEM=("MES_ORDEM", "first"),
                 CO2_TOTAL=("CO2E", "sum")))
    for uso, coluna in (("Frota", "FROTA"), ("Infra", "INFRA")):
        soma = df.loc[df["USO"] == uso].groupby("MES_REF")["CO2E"].sum()
        base[coluna] = base["MES_REF"].map(soma).fillna(0.0)
    base = base.sort_values("MES_ORDEM").reset_index(drop=True)
    return base
