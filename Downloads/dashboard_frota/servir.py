...
def _escrever_atomico(html: str) -> None:
    """Grava o HTML via arquivo temporário + os.replace para que o navegador
    nunca leia um arquivo pela metade (escrita atômica no mesmo filesystem)."""
    tmp = DIR / "dashboard_frota.html.tmp"
    tmp.write_text(html, encoding="utf-8")
    os.replace(tmp, DIR / "dashboard_frota.html")
...
def regenerar(path: Path) -> bool:
    ...
    if df.empty:
        ...
        return False
    try:
        _escrever_atomico(build_html(df))
    except Exception as e:
...
def vigiar(path: Path, intervalo: float = 1.5) -> None:
    """Vigia a planilha com debounce: só regenera quando a assinatura fica
    estável por um ciclo inteiro (evita ler o arquivo no meio do salvamento)."""
    ult = assinatura(path)
    pendente = False
    if ult is None:
        print(f"! planilha não encontrada: {path}")
    else:
        print(f"  vigiando: {path.name} (salvou -> HTML regenera sozinho)")
    while True:
        ...
        try:
            ...
            if novo == ult:
                if pendente:
                    ...
                    if regenerar(path):
                        avisar()
                        print(f"-> {path.name} mudou: dashboard atualizado às "
